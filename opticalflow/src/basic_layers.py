...
def build_resnet_block(input_res, channelsout, is_training, data_format,
                       do_batch_norm=False, use_conv_bias=True, name=None):
...
def general_conv2d(conv, name=None, channelsout=64, ksize=3, strides=2, init_factor=0.1,
                   ...
                   is_training=True, data_format=None, use_bias=True):
...
def upsample_conv2d(conv, name=None, channelsout=64, ksize=3, init_factor=0.1,
    ...
    if data_format == 'channels_first':
        conv = tf.transpose(conv, [0,2,3,1]) # [batch, channels, height, width] -> [b, h, w, c]
        shape = tf.shape(conv)
        conv = tf.image.resize_images(conv, size=[shape[1]*2, shape[2]*2], # size = [h*2, w*2]
    ...
    conv = general_conv2d(conv, name=name, channelsout=channelsout, ksize=ksize, strides=1,
                          ...
                          is_training=is_training, data_format=data_format, use_bias=use_conv_bias)
    
    return conv

def predict_flow(conv, name=None, channelsout=2, ksize=1, strides=1,
                 padding='SAME', init_factor=0.1, is_training=True,
                 data_format=None, use_conv_bias=True):

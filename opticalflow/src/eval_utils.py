...
def flow_error_dense(flow_gt, flow_pred, event_img, is_car=False):
    ...
    # Percentage outliers (using KITTY notation)
    pixel_thresh = 3.
    mag_thresh = 0.05
    f_dist = EE
    f_mag = np.linalg.norm(gt_masked, axis=-1)
    outliers = np.logical_and(f_dist > pixel_thresh, f_dist/f_mag > mag_thresh)
    n_outliers = outliers.sum()
    percent_outliers = outliers.mean()

    # Percentage of points with EE < 3 pixels.
    percent_AEE = float((EE < pixel_thresh).sum()) / float(EE.shape[0] + 1e-5)

    return AEE, percent_AEE, n_points, percent_outliers, n_outliers

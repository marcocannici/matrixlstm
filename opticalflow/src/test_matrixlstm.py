...
def test(sess,
         ...
         next_image_loader,
         timestamp_loader):
    print("Using test sequence: %s" % args.test_sequence)
    global_step = tf.train.get_or_create_global_step()
    with tf.variable_scope('vs'):
        image_shape = [args.image_height, args.image_width]
        flow_dict, matrixlstm_image_loader = matrixlstm_model(
            events_loader,
            lengths_loader,
            event_image_loader,
            1,
            args.matrixlstm_mode,
            args.matrixlstm_channels,
            args.matrixlstm_force_channels,
            image_shape,
            args.matrixlstm_region_shape,
            args.matrixlstm_region_stride,
            args.matrixlstm_actfn,
            args.matrixlstm_batchnorm,
            args.matrixlstm_maxrf,
            args.matrixlstm_keep_last,
            args.matrixlstm_time_mode,
            args.matrixlstm_norm_relative,
            args.matrixlstm_add_step,
            args.matrixlstm_add_coords,
            args.matrixlstm_frame_intervals,
            args.matrixlstm_frame_intervals_mode,
            args.matrixlstm_add_terminator_mode,
            args.matrixlstm_k_init,
            args.matrixlstm_add_selayer,
            is_training=False,
            do_batch_norm=not args.no_batch_norm,
            use_conv_bias=not args.no_conv_bias,
    ...
    while not coord.should_stop():
        ...
        if args.test_plot or args.save_test_output:
            ...
            matrixlstm_image = frame_normalize(matrixlstm_image.squeeze(0))
            matrixlstm_image_01 = to_rb(matrixlstm_image[..., [0, 1]])
            matrixlstm_image_23 = to_rb(matrixlstm_image[..., [2, 3]])

            matrixlstm_image_01 = drawImageTitle(matrixlstm_image_01, 'MatrixLSTM Chans 01')
            matrixlstm_image_23 = drawImageTitle(matrixlstm_image_23, 'MatrixLSTM Chans 23')
            ...
            top_cat = np.concatenate([matrixlstm_image_01, prev_image, pred_flow_rgb], axis=1)
            bottom_cat = np.concatenate([matrixlstm_image_23, errors, gt_flow_rgb], axis=1)
...
def main():
    ...
    sess = tf.Session()
    events_loader, lengths_loader, event_image_loader, \
    prev_image_loader, next_image_loader, timestamp_loader, n_ima = get_loader(
        args.data_path,
        1,
        args.image_width,
        args.image_height,
    ...
    if not args.load_path:
        if not '.ckpt' in args.training_instance:
            print("ERROR: TensorFlow was not able to automatically infer the checkpoint name in "
                  "the provided folder. Explicitly specify the checkpoint name, e.g., <checkpoint_folder>/last.ckpt")
        raise Exception("No checkpoint found in the provided folder!")
